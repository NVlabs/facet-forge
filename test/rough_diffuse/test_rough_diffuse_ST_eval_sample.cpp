...
int main(int argc, char **argv)
{
    ...
    if (argc != 8)
    {
        std::cout << "usage: testmirror roughx roughy gamma kd theta_i numsamplesSample numsamplesEval \n";
        exit(-1);
    }

    const float rough_x = StringToNumber<float>(std::string(argv[1]));
    const float rough_y = StringToNumber<float>(std::string(argv[2]));
    const float gamma = StringToNumber<float>(std::string(argv[3]));
    const double kd = StringToNumber<float>(std::string(argv[4]));
